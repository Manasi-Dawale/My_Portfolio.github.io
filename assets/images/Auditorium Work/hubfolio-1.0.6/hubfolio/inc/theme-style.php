<?php
function hubfolio_theme_styles()  
{ 
...
function hubfolio_fonts_url() {
	 ...
	 */
	if ( 'off' !== _x( 'on', 'Google font: on or off', 'hubfolio' ) ) {
...
*/
function hubfolio_fonts_style() {
	wp_enqueue_style( 'hubfolio-fonts', hubfolio_fonts_url(), array(), '1.0.0' );
}
add_action( 'wp_enqueue_scripts', 'hubfolio_fonts_style' );

//for google font  in editor
function hubfolio_fonts_editor_style() {
	$font_url = add_query_arg( 'family', urlencode( 'Open Sans:300,400,400i,500,600,600i,700,700i,800,800i,900|Poppins:300,400,500,600,700,800,900|Barlow Condensed:300,400,500,600,700,800,900' ), "//fonts.googleapis.com/css" );
	add_editor_style( $font_url );
}
add_action( 'after_setup_theme', 'hubfolio_fonts_editor_style' );

/*
Enqueue admin scripts and styles.
*/
function hubfolio_admin_style() {
	wp_enqueue_style('hubfolio-admin-custom', get_stylesheet_directory_uri() . '/assets/css/admin-custom.css', array(), '5.1.1', 'all' );
}
add_action( 'admin_enqueue_scripts', 'hubfolio_admin_style' );


//Register and load FontAwesome to WP Editor

add_action( 'enqueue_block_editor_assets', 'hubfolio_editor_fontawesome' );
function hubfolio_editor_fontawesome() {
